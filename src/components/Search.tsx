...
import { searchPlaces, upsertPlaceToDB } from '../services/placesService';
...
export const Search: React.FC<SearchProps> = React.memo(({ onSelect, onFocus, userLocation, userId }) => {
  ...
  const showSuggestions = isOpen && isFocused && (results.length > 0 || filteredHistory.length > 0 || isSearching);
